<template>
  <div>
    ...
    >
        ...
        <v-list-tile avatar>
          <v-list-tile-avatar class="user-div">
            <div data-v-7ba5bd90 class="v-avatar" style="width: 80px;">
              <img
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQs_tXpPKVwoWQynT736mi5UMQf8cTJeqbrBTjnIc35fU103_bQ"
                style="width:80px"
              >
            </div>
          </v-list-tile-avatar>
        ...
        <v-list-tile @click="active = 1" class :class="{'active-item': active === 1}">
          ...
          <router-link to="/dashboard-candidat">
            <v-list-tile-content @click="active = 1">
              <v-list-tile-title class :class="{'active-color': active === 1}">Tableau de bord</v-list-tile-title>
            </v-list-tile-content>
          </router-link>
        </v-list-tile>
      </v-list>
      ...
      <v-list class="pa-0">
        <v-list-group v-model="openListMenu" no-action>
          <v-list-tile slot="activator">
            ...
            <v-list-tile-content>
              <v-list-tile-title>Résultats</v-list-tile-title>
            </v-list-tile-content>
          </v-list-tile>

          <router-link to="/nouveau-resultat">
            <v-list-tile
              @click="active = 8"
              class
              :class="{'active-item active-color': active === 8 }"
            >
              <v-list-tile-content>
                <v-list-tile-title class :class="{'active-color': active === 8}">Nouveau Résultat</v-list-tile-title>
              </v-list-tile-content>
            </v-list-tile>
          </router-link>
          ...
          <router-link to="/liste-resultats">
            <v-list-tile
              @click="active = 5"
              class
              :class="{'active-item active-color': active === 5 }"
            >
              <v-list-tile-content>
    ...
    <!-- CONTENT  -->
    <v-content>
      <listeRepresentant :user="userinfo"></listeRepresentant>
    </v-content>
    <!-- END CONTENT  -->
    <!-- RIGHT MODAL  -->
    ...
    <validationModal v-if="validation" @logout="logout" @abortLogout="abortLogout"></validationModal>
  </div>
</template>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<script>
import listeRepresentant from "./ListeRepresentant.vue";
