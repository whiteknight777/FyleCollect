          ...
          <v-layout row wrap>
            ...
            <v-flex xs12 md12 lg12>
              <v-breadcrumbs divider="/" style="margin-bottom:20px">
                ...
                <v-breadcrumbs-item :disabled="false">Ajouter un résultat</v-breadcrumbs-item>
              ...
              <div>
                <v-toolbar color="blue-grey lighten-4" dark tabs style="padding-bottom: 25px;">
                  <v-autocomplete
                    :loading="loading"
                    :items="listeLieux"
                  ...
                  <v-btn
                    ...
                    @click="getLieuxData"
                  >
                    <v-icon style="margin-left: 0px;">done</v-icon>
                  </v-btn>
                ...
                <v-card v-if="lieuSelected !== null" style="padding-top:1px">
                  <v-toolbar color="blue-grey lighten-4">
                    <v-card-title style="padding-left:0px;">
                      <v-card-text style="padding-left:0px;">{{lieuSelected.name}}</v-card-text>
                    </v-card-title>
                    <v-card-actions>
                      Nombre inscrit:
                      <b>{{lieuSelected.nbInscrit}}</b>
                    </v-card-actions>
                    <v-spacer></v-spacer>
                    <v-card-actions>
                      Total Votant:
                      <b>{{lieuSelected.nbTotalVotant}}</b>
                    </v-card-actions>
                    <v-card-actions>
                      Total Bulletin null:
                      <b>{{lieuSelected.nbTotalBn}}</b>
                    </v-card-actions>
                    <v-card-actions>
                      Votant restant / Absent:
                      <b>{{lieuSelected.nbAbstention}}</b>
                    </v-card-actions>
                  </v-toolbar>
                  ...
                  <table class="v-datatable v-table">
                    ...
                    <tbody>
                      <tr v-for="(bureau, a) of lieuSelected.bv" :key="a">
                ...
                <v-card v-if="lieuSelected === null" style="padding-bottom: 21px">
                  <v-card-text>
                    <v-card-media class="text-xs-center">
                      ...
                      >how_to_vote</v-icon>
                      <v-spacer></v-spacer>
                    </v-card-media>Recherchez un lieu de vote
                  </v-card-text>
                </v-card>
              </div>
            </v-flex>
            ...
            <template>
              <v-layout row justify-center>
                <v-dialog v-model="checkData" persistent max-width="800px">
                  <v-toolbar tabs>
                  ...
                  <v-card id="addStatBureau" v-if="checkData">
                      ...
                      <v-layout wrap>
                        ...
                        <v-flex xs12 md12 lg12>
                          <v-btn
                            ...
                            :loading="loading"
                            :disabled="loading"
                            v-if="validerResultatCandidat === false && validerStatsBureau === false"
                            block
                            large
                            @click.native="updateResultatsBureau"
                          ...
                          <v-btn
                            ...
                            :loading="loading"
                            :disabled="loading"
                            v-if="(dejaValiderResultatCandidat === false && validerResultatCandidat === true) && (dejaValiderStatsBureau === false && validerStatsBureau === true)"
                            block
                            large
                            dark
                            @click.native="updateResultatsBureau"
...
export default {
  ...
  methods: {
        ...
        this.axios
          ...
          .then(response => {
            ...
            if (data.statusRequete == 100) {
              let listeBureau = this.lieuSelected.bv;
              for (let bureau of listeBureau) {
                if (bureau.id === this.bureauSelected.id) {
                  ...
                  // Vérifions si tout les résultats des candidats ont été validés
    ...
    getLieuxData() {
      this.loadingPage = true;
      this.lieuSelected = this.lieux.find(e => {
        if (e.name === this.select) return e;
      });
      ...
      this.axios
        ...
        .then(response => {
          ...
          if (data.statusRequete == 100) {
            this.lieuSelected = data.centre;
            // Vérifions si toutes les infos ont été validées
            // this.lieux.filter(e => {

            let nbTotalVotant = 0;
            let nbTotalBn = 0;
            for (let bureau of this.lieuSelected.bv) {
              let statsCandidatsBureaux = bureau.rv;
              let nbValider = 0;
              for (let resultat of statsCandidatsBureaux) {
                if (resultat.valider === true) {
            ...
            this.lieuSelected.nbTotalVotant = nbTotalVotant;
            this.lieuSelected.nbTotalBn = nbTotalBn;
            this.lieuSelected.nbAbstention =
              parseInt(this.lieuSelected.nbInscrit) - parseInt(nbTotalVotant);
    ...
    getListeLieux() {
      this.loadingPage = true;
      this.axios
        .get(apiConfig.baseURL + "centres/listLieux/" + this.user.idCandidat, {
          headers: {
            "Content-type": "application/x-www-form-urlencoded"
          }
        })
    ...
    updateResultatsBureau() {
      ...
      data.append(
        "validerResultatCandidat",
        this.validerResultatCandidat ? 1 : 0
      );
      data.append("validerStatsBureau", this.validerStatsBureau ? 1 : 0);
      ...
      // console.log("ok");
      this.axios
        .post(apiConfig.baseURL + "resultats/addbureau/info", data, {
          headers: {
            "Content-type": "multipart/form-data"
          }
        })
        .then(response => {
          let data = response.data;
          if (data.statusRequete == 100) {
            // METTONS A JOURS LES INFORMATIONS DU BUREAU SELECTIONNE
            let listeBureau = this.lieuSelected.bv;
            let nbTotalVotant = 0;
            let nbTotalBn = 0;
            let nbValider = 0;
            ...
            for (let bureau of listeBureau) {
              ...
              if (bureau.id === this.bureauSelected.id) {
                ...
                bureau.valider = this.validerStatsBureau;

                // Vérifions si les stats candidats bureaux ont été validées
                if (this.validerResultatCandidat === true) {
                  for (let resultat of statsCandidatsBureaux) {
              ...
              if (bureau.nbVotant !== " Aucun resultat ") {
                nbTotalVotant =
                  parseInt(nbTotalVotant) + parseInt(bureau.nbVotant);
              } else {
                nbTotalVotant = parseInt(nbTotalVotant) + 0;
              }

              // Récupérons le cumul des bulletin null de chaque bureaux du centre
              if (bureau.nbBulletinNull !== " Aucun resultat ") {
                nbTotalBn =
                  parseInt(nbTotalBn) + parseInt(bureau.nbBulletinNull);
              } else {
                nbTotalBn = parseInt(nbTotalBn) + 0;
              }
            }

            // METTONS A JOUR LES INFORMATIONS DU LIEU DE VOTE
            // Mettons à jours les cumuls des stats du centres
            this.lieuSelected.nbTotalVotant = nbTotalVotant;
            this.lieuSelected.nbTotalBn = nbTotalBn;
            this.lieuSelected.nbAbstention =
    ...
    checkNbVotant(v) {
      if (v > this.lieuSelected.nbInscrit) {
        this.nbVotant = this.lieuSelected.nbInscrit;
      }
